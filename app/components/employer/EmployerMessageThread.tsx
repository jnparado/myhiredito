...
export function EmployerMessageThread({ conversationId }: Props) {
  ...
  useEffect(() => {
    if (!userKey) return;
    ensureEmployerConversations(userKey);
    markEmployerConversationRead(userKey, conversationId);
  ...
  const conversation = getEmployerConversation(userKey, conversationId);
  ...
  function handleSend(e: React.FormEvent) {
    ...
    const employerName = user ? getEmployerDisplayName(user) : "You";
    sendEmployerMessage(userKey, conversationId, draft, employerName);
    setDraft("");
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            <p className="truncate text-xs text-zinc-500">
              {conversation.jobTitle ?? "Applicant"}
      ...
      <div className="flex-1 space-y-4 overflow-y-auto bg-zinc-50 px-4 py-5">
        {conversation.messages.length === 0 ? (
          <div className="rounded-lg border border-dashed border-zinc-300 bg-white px-4 py-8 text-center text-sm text-zinc-500">
            Start the conversation — reach out about their application or schedule an interview.
          </div>
        ) : (
              ...
              >
                <div
                  className={`max-w-[85%] rounded-2xl px-4 py-3 ${
                    isEmployer
                      ? "rounded-br-md bg-[#1db954] text-white"
                ...
                >
                  ...
                  <p
                    className={`mt-1 text-[10px] ${
                      isEmployer ? "text-white/70" : "text-zinc-400"
                    }`}
                  >
      ...
      >
        <div className="flex gap-2">
          <input
            ...
            className="h-11 flex-1 rounded-xl border border-zinc-200 px-4 text-sm outline-none transition focus:border-[#1db954] focus:ring-2 focus:ring-[#1db954]/20"
          />
          <button
            type="submit"
            disabled={!draft.trim()}
            className="inline-flex h-11 items-center justify-center rounded-xl bg-[#1db954] px-5 text-sm font-bold text-white transition hover:bg-[#1a5c42] disabled:opacity-40"
          >
            Send
          </button>
        </div>
