...
export function EmployerFloatingMessagingWidget() {
  const pathname = usePathname();
  const { user } = useEmployerAuth();
  const { userKey, conversations, unreadCount } = useEmployerMessages();
  ...
  const hideOnPage = pathname?.startsWith("/employer/messages");

  const activeConversation =
    userKey && activeConversationId
      ? getEmployerConversation(userKey, activeConversationId)
      : null;

  useEffect(() => {
    if (!userKey || !activeConversationId) return;
    markEmployerConversationRead(userKey, activeConversationId);
  ...
  useEffect(() => {
    ...
    window.addEventListener("myhiredito-employer-open-messaging", openMessaging);
    return () =>
      window.removeEventListener("myhiredito-employer-open-messaging", openMessaging);
  }, []);
  ...
  function handleSend(e: React.FormEvent) {
    ...
    sendEmployerMessage(userKey, activeConversationId, draft, displayName);
    setDraft("");
    setTick((value) => value + 1);
  }

  return (
    <div className="fixed bottom-0 left-0 right-0 z-[55] sm:left-auto sm:right-4 sm:w-[288px]">
      {isOpen && (
              ...
              <div className="flex items-center gap-2 border-b border-zinc-100 px-3 py-2.5">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="truncate text-[11px] text-zinc-500">
                    {activeConversation.jobTitle ?? activeConversation.participantRole}
                  </p>
                </div>
              </div>
                      ...
                      >
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <p className="mt-0.5 line-clamp-1 text-xs text-zinc-500">
                            {getEmployerConversationPreview(conversation)}
                          </p>
                        </div>
                        {unread > 0 && (
        ...
        <div className="flex items-center gap-3 px-3 py-2.5">
          ...
          >
            <div className="relative shrink-0">
              <div className="flex h-9 w-9 items-center justify-center overflow-hidden rounded-full bg-[#1db954] text-sm font-bold text-white">
          ...
          <Link
            ...
            className="flex h-8 w-8 items-center justify-center rounded-full text-zinc-600 hover:bg-zinc-100"
            aria-label="Message an applicant"
            title="Browse applicants to start a conversation"
          >
