import Link from "next/link";

type NotificationMessageItemProps = {
  title: string;
  description: string;
  onClick?: () => void;
};

export function NotificationMessageItem({
  title,
  description,
  onClick,
}: NotificationMessageItemProps) {
  return (
    <button
      type="button"
      onClick={onClick}
      className="block w-full border-b border-zinc-100 px-4 py-3 text-left transition hover:bg-zinc-50"
    >
      <p className="text-sm font-bold text-zinc-900">{title}</p>
      <p className="mt-1 text-xs leading-5 text-zinc-500">{description}</p>
    </button>
  );
}

type NotificationAlertItemProps = {
  href: string;
  title: string;
  subtitle: string;
  status: string;
  onClick?: () => void;
};

export function NotificationAlertItem({
  href,
  title,
  subtitle,
  status,
  onClick,
}: NotificationAlertItemProps) {
  return (
    <Link
      href={href}
      onClick={onClick}
      className="block border-b border-zinc-100 px-4 py-3 transition hover:bg-red-50/60"
    >
      <div className="flex items-start gap-3">
        <span className="mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-red-600 text-[10px] font-bold text-white">
          !
        </span>
        <div>
          <p className="text-sm font-bold text-red-800">{title}</p>
          <p className="mt-1 text-xs leading-5 text-red-700">{subtitle}</p>
          <p className="mt-1 text-[11px] text-zinc-500">{status}</p>
        </div>
      </div>
    </Link>
  );
}

type NotificationPanelProps = {
  unreadMessages: number;
  onboardingIncomplete: boolean;
  nextStep?: { step: number; label: string; href: string };
  completed: number;
  total: number;
  onClose: () => void;
  onOpenMessages: () => void;
};

export function NotificationPanel({
  unreadMessages,
  onboardingIncomplete,
  nextStep,
  completed,
  total,
  onClose,
  onOpenMessages,
}: NotificationPanelProps) {
  const hasContent = unreadMessages > 0 || onboardingIncomplete;

  return (
    <div className="fixed inset-x-3 top-[calc(100%+0.5rem)] z-50 w-auto overflow-hidden rounded-xl border border-zinc-200 bg-white text-zinc-900 shadow-xl sm:absolute sm:inset-x-auto sm:right-0 sm:w-80">
      <div className="border-b border-zinc-100 px-4 py-3">
        <h3 className="text-sm font-bold text-zinc-900">Notifications</h3>
      </div>

      {hasContent ? (
        <div>
          {unreadMessages > 0 && (
            <NotificationMessageItem
              onClick={() => {
                onClose();
                onOpenMessages();
              }}
              title={`${unreadMessages} new message${unreadMessages === 1 ? "" : "s"}`}
              description="Employers and recruiters are trying to reach you."
            />
          )}

          {onboardingIncomplete && nextStep && (
            <NotificationAlertItem
              href={nextStep.href}
              onClick={onClose}
              title="Complete your account setup"
              subtitle={`Step ${nextStep.step} of ${total}: ${nextStep.label}`}
              status={`${completed}/${total} onboarding steps done`}
            />
          )}
        </div>
      ) : (
        <div className="px-4 py-8 text-center">
          <p className="text-sm font-medium text-zinc-700">You&apos;re all caught up</p>
          <p className="mt-1 text-xs text-zinc-500">No new notifications right now.</p>
        </div>
      )}

      <div className="space-y-1 border-t border-zinc-100 px-4 py-3">
        {onboardingIncomplete && (
          <Link
            href="/worker/dashboard"
            onClick={onClose}
            className="block text-sm font-semibold text-[var(--brand)] hover:underline"
          >
            View all onboarding tasks
          </Link>
        )}
        <button
          type="button"
          onClick={() => {
            onClose();
            onOpenMessages();
          }}
          className="block text-sm font-semibold text-[var(--brand)] hover:underline"
        >
          Open messages
        </button>
      </div>
    </div>
  );
}
